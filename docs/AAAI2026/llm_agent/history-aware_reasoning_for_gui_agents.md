...
3. **混合 RL 奖励函数**：
   ...
   - **Action Reward** $r^{action}$：对坐标类动作（CLICK），使用多尺度欧几里得距离奖励（归一化坐标距离 + 绝对坐标距离），鼓励精细屏幕感知
   - **Memory-Augmented Reward (MAR)** $r^{memory}$：用模型判断 CoT 中是否包含对历史交互的分析。这是关键创新——显式奖励"在推理中考虑了前面做了什么"
   - 混合：$r = r^{format} \times (r^{action} + \gamma \times r^{memory})$
