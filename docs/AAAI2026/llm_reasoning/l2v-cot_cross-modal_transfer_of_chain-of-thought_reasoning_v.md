...
## 亮点
- **频域视角的深刻洞见**：发现 VLM 和 LLM 的 CoT 表示在低频分量上高度一致，高频差异来自多模态训练引入的噪声。这为跨架构能力迁移提供了理论依据
- **真正的跨架构 training-free 迁移**：不要求 VLM 和 LLM 共享 backbone，通过频域重采样优雅解决维度不匹配
- **超越有监督方法**：L2V-CoT（training-free）在多个 benchmark 上超过了 Finetuned CoT（有监督），说明直接操作隐藏表示比微调更高效
- **即插即用**：可与显式推理方法（如 MCTS）互补组合

## 局限性 / 可改进方向
- 注入层和注入强度需要 per-task 调优（Table S.2/S.3 显示不同任务用不同超参）
- 只在数学推理 benchmark 上验证，未测试通用 VQA 或更广泛的推理任务
- CoT 样本来自 STILL-2 数据集（数学/物理/化学/生物），领域偏差可能影响泛化
- 频域重采样假设 CoT 信息集中在低频，不清楚在所有任务上是否成立
...
- 写作质量: ⭐⭐⭐⭐ 经验分析→方法设计的逻辑清晰，可视化有说服力
- 价值: ⭐⭐⭐⭐ 提供了一种通用的跨架构推理迁移方案，对 VLM 推理增强有实际意义
