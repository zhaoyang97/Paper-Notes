...
description: >-
  [ICLR 2026][多模态VLM][视觉语言检索] 提出EDJE（高效判别式联合编码器），通过将视觉特征提取离线化并用轻量级注意力适配器压缩视觉Token，实现50k图文对/秒的高吞吐推理，同时在Flickr（零样本）和COCO（微调）检索上匹配现有联合编码器的性能，每张图仅需49kB存储。 大规模多模态检索中…
tags:
  - "ICLR 2026"
  - "多模态VLM"
  - "视觉语言检索"
