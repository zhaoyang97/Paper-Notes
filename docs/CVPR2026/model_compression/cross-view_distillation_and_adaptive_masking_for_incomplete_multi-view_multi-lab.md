...
description: >-
  [CVPR 2026][模型压缩][不完整多视图] 针对"视图与标签双重缺失"的多标签分类，本文用一个强视图当 teacher 去蒸馏其余弱视图、再用一个可学习的二值门控把蒸馏后仍然不可靠的视图直接屏蔽掉，在六个数据集上稳定超过九个 SOTA。 领域现状：不完整多视图多标签学习（iM2L）要在"视图缺失（传感器故障）+ 标签…
tags:
  - "CVPR 2026"
  - "模型压缩"
  - "不完整多视图"
