...
- 价值: ⭐⭐⭐⭐ 为具身智能的场景理解提供了新范式，开源代码将促进后续研究
**领域**: 3D视觉 / NeRF / 3DGS / 自动驾驶 / 具身智能  
**关键词**: Open, O3N, Omnidirectional, mamba, embodied  

## 一句话总结
通过全方位感知来理解和重构3D世界是自主智能体和仿真发展的必然趋势。

## 背景与动机
通过全方位感知来理解和重构3D世界是自主智能体和具身智能发展的必然趋势。然而，现有的 3D 占用预测方法受到有限的视角输入和预定义的训练分布的限制，使得它们难以应用于开放世界探索中需要全面且安全地感知场景的实体代理。
## 核心问题
通过全方位感知来理解和重构3D世界是自主智能体和具身智能发展的必然趋势。然而，现有的 3D 占用预测方法受到有限的视角输入和预定义的训练分布的限制...

## 方法详解

### 整体框架
为了解决这个问题，我们提出了 O3N，第一个纯视觉的、端到端的全方位开放词汇占用预测框架。
### 关键设计
1. **设计1**: 为了解决这个问题，我们提出了 O3N，第一个纯视觉的、端到端的全方位开放词汇占用预测框架。
2. **设计2**: O3N 通过极螺旋 Mamba (PsM) 模块将全向体素嵌入极螺旋拓扑中，从而实现 360° 的连续空间表示和远程上下文建模。
3. **设计3**: 占用成本聚合（OCA）模块引入了一种原则性机制，用于统一体素空间内的几何和语义监督，确保重建的数据之间的一致性
4. **设计4**: 此外，自然模态对齐（NMA）建立了一种无梯度对齐路径，协调视觉特征、体素嵌入和文本语义，形成一致的“像素-体素-文本”
...
## 与我的研究方向的关联
- 可能关联: `20260316_multi_source_ov_det.md`
