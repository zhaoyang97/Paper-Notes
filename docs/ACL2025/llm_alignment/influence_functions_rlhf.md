...
| 长度偏差检测 | GPT-4o | 0.747 | TPR 比本文低 5.3% |
| 长度偏差检测 | Gemini-1.5-Pro | — | TPR 比本文低 25.6% |
| 长度偏差检测 | Mahalanobis / KNN | ~0.600 | 接近随机 |
| 谄媚偏差检测 | **IF (本文)** | **0.711** | 最优 |
| 谄媚偏差检测 | GPT-4o | — | TPR 比本文低 14.8% |
| 谄媚偏差检测 | 其他基线 | ~0.600 | 接近随机 |

### 错标检测人工验证（Anthropic-HH 原始数据）

| 采样策略 | 错标 | 正确 | 平局 |
|----------|------|------|------|
| Top-100（影响分数最低） | **47** | 38 | 15 |
| Random-100 | 13 | 69 | 18 |
...
| 配置 | 计算时间 | 存储需求 | 说明 |
|------|----------|----------|------|
| DataInf（无压缩） | 28.8 小时 | 16TB (10^5 样本) | 需 2 次反向传播 |
| DataInf + OPORP (本文) | 11.1h 压缩 + 92.3s 计算 | 256KB/样本 | 仅需 1 次反向传播 |
| 梯度维度 | 42M → 65K | 压缩比 ~625x | 保持点积不变 |

### 标注策略指导实验（HelpSteer2 数据集）

| 指标 | 更新前 (Bob) | 更新后 (IF) | 更新后 (KNN) | 更新后 (Mahal.) |
|------|-------------|-------------|-------------|-----------------|
| Label Acc. | 基准 | +15.8% | 下降 | 下降 |
| RM Acc. | 基准 | +2.2% | 下降 | 下降 |
| Cos Sim. (与Alice) | 基准 | +0.45 | 下降 | 下降 |
